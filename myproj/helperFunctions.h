...
enum {
  ...
  BUFFER_OVERLAY_VERTICES,
  BUFFER_OVERLAY_NORMALS
};
enum {
...
static string resolve_resource_path(const string &relative) {
  ...
  for (size_t i = 0; i < candidates.size(); i++) {
    // std::cout << "Checking candidate: " << candidates[i] << " -> " <<
    // (file_exists(candidates[i]) ? "EXISTS" : "NOT FOUND") << std::endl;
    if (file_exists(candidates[i])) {
      // std::cout << "Selected: " << candidates[i].lexically_normal().string()
      // << std::endl;
...
static bool ray_intersects_triangle(const glm::vec3 &origin,
...
static bool build_picking_ray(int x, int y, glm::vec3 &ray_origin,
                              glm::vec3 &ray_direction) {
  if (window_w <= 0 || window_h <= 0)
    return false;

  const float ndc_x =
      (2.0f * static_cast<float>(x) / static_cast<float>(window_w)) - 1.0f;
  const float ndc_y =
      (2.0f * static_cast<float>(y) / static_cast<float>(window_h)) - 1.0f;

  const glm::mat4 projection_matrix = glm::perspective(
      glm::radians(fovy),
      static_cast<float>(window_w) / static_cast<float>(window_h), zNear, zFar);
  ...
  const glm::mat4 inv_view_projection =
      glm::inverse(projection_matrix * view_matrix);
  ...
  if (std::fabs(near_world.w) < 1e-7f || std::fabs(far_world.w) < 1e-7f)
    return false;
...
void makeBuffers(myMesh *input_mesh) {
  ...
  glBufferData(GL_ARRAY_BUFFER, norms.size() * sizeof(GLfloat),
               norms.empty() ? nullptr : norms.data(), GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, norms_per_face.size() * sizeof(GLfloat),
               norms_per_face.empty() ? nullptr : norms_per_face.data(),
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, verts_and_normals.size() * sizeof(GLfloat),
               verts_and_normals.empty() ? nullptr : verts_and_normals.data(),
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices_edges.size() * sizeof(GLuint),
               indices_edges.empty() ? nullptr : indices_edges.data(),
               GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[BUFFER_INDICES_VERTICES]);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               indices_vertices.size() * sizeof(GLuint),
               indices_vertices.empty() ? nullptr : indices_vertices.data(),
               GL_STATIC_DRAW);
...
bool PickedPoint(int x, int y) {
  if ((x < 0) || (window_w <= x) || (y < 0) || (window_h <= y))
    return false;
  if (m == NULL || m->faces.empty())
    return false;

  glm::vec3 ray_origin;
  glm::vec3 ray_direction;
  if (!build_picking_ray(x, y, ray_origin, ray_direction))
    return false;
  ...
  for (vector<myFace *>::iterator fit = m->faces.begin(); fit != m->faces.end();
       ++fit) {
    myFace *face = *fit;
    ...
    const glm::vec3 p0(static_cast<float>(e0->source->point->X),
    ...
    while (e != NULL && e->next != NULL && e->next != e0 && guard < 100000) {
      if (e->source != NULL && e->source->point != NULL &&
        ...
        const glm::vec3 p1(static_cast<float>(e->source->point->X),
                           static_cast<float>(e->source->point->Y),
                           static_cast<float>(e->source->point->Z));
        const glm::vec3 p2(static_cast<float>(e->next->source->point->X),
                           static_cast<float>(e->next->source->point->Y),
                           static_cast<float>(e->next->source->point->Z));

        float t = 0.0f;
        if (ray_intersects_triangle(ray_origin, ray_direction, p0, p1, p2, t) &&
            t < best_t) {
          best_t = t;
          best_face = face;
...
void initInterface(int argc, char *argv[]) {
  if (argc > 0 && argv != NULL && argv[0] != NULL) {
    ...
    if (ec)
      g_executable_path = std::filesystem::path(argv[0]);
  }
  ...
  g_window = SDL_CreateWindow("Mesh Viewer", SDL_WINDOWPOS_CENTERED,
  ...
  glGetError(); // GLEW can raise a harmless GL_INVALID_ENUM once on core
                // contexts.
  ...
  while (g_running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_MOUSEBUTTONUP:
        mouse(event.button.button, event.button.state, event.button.x,
              event.button.y, SDL_GetModState());
        break;
